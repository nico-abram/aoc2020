...
const DATA: &str = "eyr:2028 iyr:2016 byr:1995 ecl:oth
...
fn main() {
    ...
    for line in DATA.lines() {
        if line.is_empty() {
            let mut valid = [&byr, &iyr, &eyr, &hgt, &hcl, &ecl, &pid /*, &cid*/]
                .iter()
                .all(|&x| x.is_some());
            valid = valid
                && pid
                    .map(|pid: &str| pid.len() == 9 && pid.parse::<usize>().is_ok())
                    .unwrap_or(false);
            valid = valid
                && ecl
                    .map(|ecl: &str| {
            ...
            valid = valid
                && hcl
                    .map(|hcl: &str| {
                        hcl.get(0..1) == Some("#") && hcl[1..].chars().all(|x| x.is_alphanumeric())
                    })
                    .unwrap_or(false);
            valid = valid
                && hgt
                    .map(|hgt: &str| {
                        ...
                        let r = hgt[..len-2].parse::<usize>();
                        if !r.is_ok() {return false;}
                        let r = r.unwrap();
                        if &hgt[len-2..] == "cm" {r >= 150 && r <= 193}else if &hgt[len-2..] == "in"{r >= 59 && r <= 76} else {false}
                    })
                    .unwrap_or(false);
            valid = valid
                && eyr
                    .map(|eyr: &str| {
                        let x = eyr.parse::<usize>();
                        let x = x.as_ref();
                        eyr.len() == 4 && x.is_ok() && x.unwrap() <= &2030 && x.unwrap() >= &2020
                    })
                    .unwrap_or(false);
            valid = valid
                && iyr
                    .map(|iyr: &str| {
                        let x = iyr.parse::<usize>();
                        let x = x.as_ref();
                        iyr.len() == 4 && x.is_ok() && x.unwrap() <= &2020 && x.unwrap() >= &2010
                    })
                    .unwrap_or(false);
            valid = valid
                && byr
                    .map(|byr: &str| {
                        let x = byr.parse::<usize>();
                        let x = x.as_ref();
                        byr.len() == 4 && x.is_ok() && x.unwrap() <= &2002 && x.unwrap() >= &1920
                    })
                    .unwrap_or(false);
            if valid {
                valid_passports += 1;
    ...
    println!("valid_passports part 2: {}", valid_passports);
}
