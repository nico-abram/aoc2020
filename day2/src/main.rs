...
of b, but needs at least 1. The first and third passwords are valid: they contain one a or nine c, both within the limits of their respective policies.
*/
const DATA: &str = include_str!("input.data");

fn main() {
    const INVALID_FORMAT: &str = "invalid line format, expected `N-M x: password`";
    ...
    let parsed_data = DATA
        ...
        .map(|line| {
            ...
            let policy = parts.next().expect("this should never happen");
            let password = parts.next().expect(INVALID_FORMAT);

            let mut policy_parts = policy.splitn(2, "-");
            let min = policy_parts
                .next()
                .expect(INVALID_FORMAT)
                .parse::<usize>()
                .expect(INVALID_FORMAT);
            let mut policy_parts2 = policy_parts.next().expect(INVALID_FORMAT).splitn(2, " ");
            let max = policy_parts2
                .next()
                .expect(INVALID_FORMAT)
                .parse::<usize>()
                .expect(INVALID_FORMAT);
            let char = policy_parts2
                .next()
                .expect(INVALID_FORMAT)
                .chars()
                .next()
            ...
            //println!("{}-{} {}: {}", min, max, char, password);
            (min, max, char, password)
        })
        .collect::<Vec<_>>();

    for (min, max, char, password) in &parsed_data {
        let charcount = &password.chars().filter(|ch| ch == char).count();
